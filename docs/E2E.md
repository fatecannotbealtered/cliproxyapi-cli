...
The uncommitted updater development tree, then designated as the `1.0.2` candidate, passed isolated standalone and npm-managed update E2E against the real signed/public `1.0.1` release. That run was development evidence only, so `1.0.2` was never promoted past `beta`.

The clean `1.0.3` candidate at `1b2393c` closed that gap on 2026-08-10. A candidate-bound disposable-instance E2E passed 29 of 29 checks across the whole command surface, and the standalone, npm-managed, and already-current self-update scenarios all passed against the real signed `1.0.1` release. The synthetic Codex record cannot produce structured provider evidence; since `internal/quota/codex.go` is unchanged in `1.0.3` and the guard's observation path is behavior-identical after the apply-mode removal, the `1.0.0` full production E2E and the targeted `1.0.1` quota smoke remain the qualifying evidence for that surface. Together these qualify `1.0.3` for `stable`.
...
| 2026-08-10 | `1.0.3` / `1b2393c` / binary `20EBE835...DD2F79` | Disposable container `v7.2.120` / `ea37d13`, loopback-only | Windows amd64 | Whole command surface on a disposable instance with a synthetic Codex record: self-description, listing, conservative quota classification, observation-only guard, all three write-gate rejections, dangerous dry-run/confirm write with re-read and restoration, token replay, dry-run target validation, raw path rejection and body omission, login/logout keyring lifecycle, redaction | Pass; 29/29 checks. No positive provider evidence — `1.0.1` production smoke retained for that surface | [Sanitized evidence](e2e/2026-08-10-1.0.3-1b2393c-disposable.md) |
...
| 2026-08-10 | Clean `1.0.3` candidate `1b2393c`; updater binary version-injected as `1.0.0`, SHA-256 `E1E07932...BC5848` | Isolated Windows amd64 standalone path and isolated npm prefix | Real signed GitHub Release and public npm packages for `1.0.1` | Pass. Standalone: `signature_status=verified`, checksum verified, atomic replacement, launched as `1.0.1`, post-update `changelog`/`reference` read cleanly. npm: `install_method` detected, manager driven, wrapper and platform metadata reached `1.0.1`. Already-current: `up_to_date`, `binary_replaced=false`, no package-manager command re-run, Skill still synchronized. All three reported `skill_sync_status=synced` | Candidate-bound. Qualifies `1.0.3` for `stable`. No Management endpoint, credential, or account data was used. |
