...
Run through this gate **before the first public push** of `cliproxyapi-cli`. It is a security and quality checkpoint, not documentation — every box must be ticked (or consciously waived with a note) before the repo goes public. Once public, secrets in history cannot be un-leaked.
...
- [ ] `.env`, `*.local`, credential files, and `~/.cliproxyapi-cli/` artifacts are listed in `.gitignore` and confirmed untracked (`git status --ignored`).
...
- [ ] Install blocks are copy-paste-runnable and use the real published `@fateforge/cliproxyapi-cli` / `fatecannotbealtered/cliproxyapi-cli`.
...
- [ ] If `cliproxyapi-cli` wraps a third-party product (CLIProxyAPI), `NOTICE.md` carries the trademark / non-affiliation notice and `docs/COMPATIBILITY.md` lists the verified backend version matrix.
...
- [ ] `skills/cliproxyapi-cli/SKILL.md` is present; frontmatter includes `version`, `license: MIT`, `user-invocable: true`, and `metadata.requires.min_version` matching the CLI version.
...
- [ ] `skills/cliproxyapi-cli/test-prompts.json` is present, valid JSON, and covers fresh-agent read, write safety or read-only boundary, permission boundary, `_untrusted` handling, and self-update.
- [ ] A bare `update` (single command, no confirm token) syncs the whole `skills/cliproxyapi-cli/` directory or returns a `skill_sync_command` equivalent to `npx skills add fatecannotbealtered/cliproxyapi-cli -y -g`; failures/interruptions report `stage` + `current_version` + `binary_replaced` + `skill_sync_status`.
- [ ] `cliproxyapi-cli reference`, `cliproxyapi-cli context`, and `cliproxyapi-cli doctor` run and emit valid JSON envelopes — an agent can self-onboard from a clean checkout.
- [ ] `cliproxyapi-cli reference` exposes `release_readiness`, and `cliproxyapi-cli doctor` reports the matching check.
