# Contributing to cliproxyapi-cli
...
```bash
# Clone
git clone https://github.com/fatecannotbealtered/cliproxyapi-cli.git
cd cliproxyapi-cli

# --- Go variant ---
go mod download
make build                      # or: go build -o bin/cliproxyapi-cli ./cmd/cliproxyapi-cli
go test -race ./...
./bin/cliproxyapi-cli --help

# --- Python variant ---
# pip install -e ".[dev]"
# python build.py
# pytest tests/ -v
# cliproxyapi-cli --help
...
Release standard: **Functional Contract Coverage = 100%**. Every public behavior documented in `README`, `SKILL`, reference pages, `cliproxyapi-cli reference`, `--help`, `context`, `doctor`, `changelog`, or `update` must have automated command-level tests.
...
Keep `cliproxyapi-cli reference` `release_readiness` and `cliproxyapi-cli doctor`'s `release_readiness` check honest when test evidence changes.
...
5. **SKILL** — add a `skills/cliproxyapi-cli/reference/<domain>.md` page and link it from `skills/cliproxyapi-cli/SKILL.md` (keep SKILL.md a short progressive-disclosure index).
6. **Docs** — update `README.md` / `README_zh.md` command lists and add a line under `## [Unreleased]` in `CHANGELOG.md`.

`reference` walks the command tree automatically, so new commands appear in `cliproxyapi-cli reference` without extra wiring.
