...
function packageLock(root) {
  ...
  // Each platform package appears as an optionalDependencies pin and as its own
  // packages["node_modules/<name>"] entry. Keep both versions synchronized.
  // Do not run `npm install --package-lock-only`, which could inject a developer
  // registry mirror into the public lockfile.
  return {
